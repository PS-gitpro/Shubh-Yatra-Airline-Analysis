...
def main():
    """Main application function for Shubh Yatra - Airline Risk Analysis Dashboard"""
    ...
    def generate_sample_data():
        ...
        Returns:
            pandas.DataFrame: Sample flight data with incidents and risk information
        """
    ...
    def calculate_risk_score(airline: str, df: pd.DataFrame) -> float:
        """
    ...
    elif analysis_type == "Aircraft Type Risk":
        st.markdown("### Risk by Aircraft Type")
        
        # Calculate risk by aircraft type
        aircraft_risk = df.groupby('Aircraft').apply(
            lambda x: calculate_risk_score(x.name, df) if len(x) > 0 else 0
        ).reset_index(name='Risk_Score')
        
        fig = px.bar(aircraft_risk, x='Aircraft', y='Risk_Score',
                     title='Risk Score by Aircraft Type')
        st.plotly_chart(fig, use_container_width=True)

    elif analysis_type == "Time of Day Risk Analysis":
        st.markdown("### Risk by Time of Day")
        
        time_risk = df.groupby('Time_of_Day').apply(
            lambda x: len(x[x['Incident_Type'] != 'None']) / len(x) * 100 if len(x) > 0 else 0
        ).reset_index(name='Incident_Rate')
        
        fig = px.bar(time_risk, x='Time_of_Day', y='Incident_Rate',
                     title='Incident Rate by Time of Day')
        st.plotly_chart(fig, use_container_width=True)
    ...
    if st.button("Generate Risk Report"):
        with st.spinner("Generating comprehensive risk report..."):
            ...
            report_data['Analysis Period'] = time_period
            report_data['Report Generated'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            
            csv = report_data.to_csv(index=False)
            st.download_button(
                label="Download Risk Analysis Report (CSV)",
                data=csv,
                file_name=f"shubh_yatra_risk_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                mime="text/csv"
            )
    ...
    with st.expander("How to use this dashboard"):
        ...
        **Note**: When using your own CSV data, include columns for:
        - Date, Flight, Airline, Aircraft, Incident_Type, Severity
        - Optional: Risk_Score (pre-calculated risk scores)
        """)
